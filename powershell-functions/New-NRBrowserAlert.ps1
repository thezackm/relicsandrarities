...
<#
.SYNOPSIS 
    Function to create New Relic Browser Alert
...
.EXAMPLE
    New-NRBrowserAlert -AdminAPIKey '123456789ABCDEFGHIJ' -PolicyID '987654321' -AlertName 'testAlert' -Enabled true -Entities 9999999 `
    -Metric total_page_load -ConditionScope application -ViolationCloseTimer 24 -TermsDuration 120 -TermsOperator above -TermsPriority critical `
    -TermsThreshold 15 -TermsTimeFunction all -RunbookURL 'http://runbook.com'
...
Function New-NRBrowserAlert {
    ...
    Param (
        ...
        [ Parameter ( Mandatory = $true ) ] [ ValidateSet( 'end_user_apdex', 'total_page_load', 'page_rendering', 'web_application', 'network', 'dom_processing', 'request_queuing', 'ajax_response_time', 'page_views_with_js_errors', 'page_view_throughput', 'ajax_throughput', 'user_defined' ) ] [ string ] $Metric,
